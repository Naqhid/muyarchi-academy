...
import { BookOpen, FileText, Calendar, Star, Image, TrendingUp, Plus } from 'lucide-react'
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Skeleton } from '@/components/ui/skeleton'
import { fetchCourses, fetchBlogs, fetchEvents, fetchTestimonials, fetchMedia } from '@/lib/api'

export default function Dashboard() {
  const [counts, setCounts] = useState({ courses: 0, blogs: 0, events: 0, testimonials: 0, media: 0 })
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all([fetchCourses(), fetchBlogs(), fetchEvents(), fetchTestimonials(), fetchMedia()])
      .then(([c, b, e, t, m]) => { setCounts({ courses: c.length, blogs: b.length, events: e.length, testimonials: t.length, media: m.length }) })
